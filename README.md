# YueEx — Bộ Công Cụ Thử Nghiệm & Phát Triển Luau Cao Cấp (Bypass EDR/Anti-Cheat)

**YueEx** là một công cụ phát triển và kiểm thử nội bộ dành cho ứng dụng **MyApp**, được thiết kế để nạp mã nguồn Luau trực tiếp vào tiến trình runtime của MyApp mà không thông qua loader chuẩn của hệ thống. Dự án kết hợp hiệu năng vượt trội và tính năng can thiệp cấp hệ thống của **Rust** với giao diện trực quan của **C# WPF**.

Dự án này sử dụng các kỹ thuật bypass nâng cao cấp độ Ring 3 (User-mode) nhằm vượt qua các cơ chế kiểm tra bộ nhớ, luồng và tải mô-đun của các hệ thống EDR / Anti-Cheat hiện đại (ví dụ: Byfron/Hyperion).

---

## 📌 Kiến Trúc Hệ Thống (System Architecture)

Dự án bao gồm hai thành phần chính:
1. **`YueEx.exe` (Out-of-process - Bộ điều khiển & Giao diện)**:
   - Được viết bằng **C# WPF** kết hợp với engine lõi bằng **Rust** (`yueex_engine.dll`).
   - Nhiệm vụ: Tìm kiếm/lựa chọn tiến trình `MyApp.exe`, thực hiện nạp thủ công (Manual Mapping) `YueEx.dll`, cung cấp trình soạn thảo script Luau và nhận telemetry/kết quả thực thi trả về.
2. **`YueEx.dll` (In-process - Trình chạy nội bộ)**:
   - Được phát triển hoàn toàn bằng **Rust** (`yueex-dll`).
   - Nhiệm vụ: Chạy ngầm trong không gian bộ nhớ của `MyApp.exe`, giao tiếp qua kênh IPC siêu ẩn, liên kết với Task Scheduler của Luau VM và thực thi script an toàn.

```text
YueEx.exe (WPF UI)
   │
   ├─► Cầu nối C-ABI (yueex_engine.dll)
   │     │
   │     ├─► Manual Mapping (Module Overloading để đạt MEM_IMAGE)
   │     │     │
   │     │     └─► Nạp YueEx.dll vào MyApp.exe
   │     │
   │     └─► Thiết lập IPC ẩn danh (Shared Memory + Named Events)
   │           ▲
   ▼           │ (Gửi Script & Nhận kết quả)
YueEx.dll (Trong tiến trình MyApp)
   │
   ├─► Cloaking & Stealth (Xóa PE Headers, Spoof Call Stack, Indirect Syscalls)
   │
   └─► Luau Runtime Integration (Hook task scheduler & thực thi bytecode)
```

---

## 🛠️ Công Nghệ & Kỹ Thuật Lõi (Core Technologies)

### 1. Kỹ Thuật Nạp Bộ Nhớ & Tránh Phát Hiện (Injection & Memory Strategy)
Để tránh bị phát hiện bởi cơ chế kiểm soát của `LdrRegisterDllNotification` và hệ thống giám sát bộ nhớ:
- **Manual Mapping (`yueex-injector`)**: Tự động parse cấu trúc PE (Portable Executable) của DLL từ byte array trong bộ nhớ. Tự ánh xạ các section, giải quyết bảng nhập khẩu (Import Address Table - IAT), và áp dụng base relocation thủ công mà không sử dụng bất kỳ API nạp mô-đun chuẩn nào của Windows (`LoadLibrary` / `LdrLoadDll`).
- **Module Overloading**: Thay vì cấp phát vùng nhớ riêng tư (`MEM_PRIVATE` - vốn là mục tiêu quét hàng đầu của anti-cheat), YueEx sẽ tải một DLL hệ thống hợp lệ và đã được ký của Windows, sau đó ghi đè mã nguồn của `YueEx.dll` lên đó. Điều này giúp bộ nhớ của YueEx đạt trạng thái `MEM_IMAGE` (Mô phỏng ảnh đĩa hợp lệ).
- **Thread Hijacking**: Tránh tạo luồng mới bằng `CreateThread` (dễ bị phát hiện start address lạ). Thay vào đó, YueEx bắt giữ luồng đang chạy hợp lệ của ứng dụng khách (Thread Hijacking) để gọi `DllMain` và hàm khởi tạo trì hoãn (`yueex_deferred_init`).

### 2. Bộ Công Cụ Stealth Tối Thượng (`yueex-stealth`)
Triển khai các kỹ thuật ẩn mình nâng cao trong chế độ User-mode:
- **Indirect Syscalls**: Thực hiện các lời gọi hệ thống NT syscall trực tiếp bằng cách nhảy đến các opcode `syscall` sạch nằm trong `ntdll.dll`, bỏ qua các hook EDR đặt ở đầu các hàm API.
- **Stack Spoofing**: Giả mạo Call Stack của luồng thực thi để nó trông như được gọi từ các mô-đun hệ thống có chữ ký số của Microsoft.
- **Memory Cloaking**: Che giấu vùng nhớ thực thi bằng cách sử dụng cơ chế Vectored Exception Handling (VEH) kết hợp với các trang nhớ `PAGE_GUARD` thực thi một bước.
- **API Hashing**: Toàn bộ API hệ thống được phân giải động qua mã băm (hash value) tại thời điểm runtime, loại bỏ hoàn toàn các chuỗi ký tự thô (plaintext) về tên hàm trong tệp nhị phân.
- **Thread Pool Hijacking**: Đăng ký các công việc thực thi vào Thread Pool Worker có sẵn của Windows thay vì tạo các luồng độc lập.

### 3. Giao Tiếp Liên Tiến Trình Siêu Ẩn (Stealthy IPC Channel)
- Kênh giao tiếp IPC sử dụng kết hợp giữa **Bộ nhớ dùng chung (Shared Memory)** và **Sự kiện có tên (Named Events)**.
- **Dynamic Names**: Để tránh bị phát hiện thông qua việc liệt kê (enumeration) các đối tượng kernel có tên cố định, các tên của Shared Memory và Events được tính toán động dựa trên mã tiến trình (PID).
- Tốc độ truyền tải cực cao với độ trễ tối thiểu, cho phép truyền script Luau và lấy dữ liệu stdout/stderr tức thời.

### 4. Tích Hợp Luau VM (`yueex-engine` / `yueex-dll`)
- **Pattern Scanning**: Quét chữ ký nhị phân (AOB scanning) trong bộ nhớ của MyApp để tìm vị trí của cấu trúc dữ liệu `lua_State`, các hàm biên dịch bytecode Luau (`luau_load`) và hàm thực thi (`lua_pcall`).
- **Scheduler Hooking**: Đồng bộ hóa việc chạy script với vòng lặp tác vụ (task scheduler) chính của MyApp để đảm bảo an toàn cho luồng xử lý và không gây crash ứng dụng.

### 5. Giao Diện Người Dùng WPF (`YueExUI`)
- Viết bằng công nghệ **.NET / C# WPF** hiện đại.
- Sử dụng **P/Invoke (Platform Invoke)** để gọi các API C-ABI xuất khẩu từ thư viện Rust `yueex_engine.dll`.
- Giao diện trực quan bao gồm:
  - Bộ chọn tiến trình đích thông minh.
  - Nút kích hoạt Manual Mapping và hiển thị trạng thái log chi tiết từng bước.
  - Trình soạn thảo mã Luau chuyên dụng với bảng điều khiển kết quả hiển thị thời gian chạy tính bằng micro giây ($\mu s$).

---

## 📂 Cấu Trúc Thư Mục Dự Án (Project Structure)

```text
├── .cargo/               # Cấu hình compiler Rust
├── crates/               # Các crate cấu thành nên engine Rust
│   ├── yueex-abi/        # Định nghĩa các cấu trúc dữ liệu dùng chung và IPC protocol
│   ├── yueex-pe/         # Thư viện parse định dạng Portable Executable (PE)
│   ├── yueex-injector/   # Lõi Manual Mapper & nạp DLL vào tiến trình
│   ├── yueex-ipc/        # Kênh IPC Shared Memory & Named Events
│   ├── yueex-stealth/    # Bộ công cụ ẩn mình (Syscalls, Stack Spoofing, VEH)
│   ├── yueex-engine/     # Cầu nối C-ABI cung cấp API cho UI C#
│   ├── yueex-dll/        # Dự án biên dịch ra YueEx.dll (In-process runtime)
│   ├── cli-injector/     # Tool CLI phục vụ việc test injection nhanh
│   └── mock-host/        # Tiến trình giả lập MyApp để debug phát triển
├── YueExUI/              # Dự án giao diện C# WPF (.NET)
├── docs/                 # Tài liệu phân tích và thiết kế chi tiết
│   ├── myapp-verifier.md # Cơ chế bảo mật và phát hiện của MyApp
│   └── project-scope.md  # Phạm vi và kiến trúc giải pháp
├── YueEx.sln             # Solution của Visual Studio
└── Cargo.toml            # Quản lý workspace dependencies của Rust
```

---

## ⚡ Hướng Dẫn Build Nhanh (Build Guide)

Dự án yêu cầu cài đặt **Rust toolchain** (mục tiêu `x86_64-pc-windows-msvc`) và **.NET SDK** mới nhất.

1. **Biên dịch toàn bộ workspace Rust**:
   ```powershell
   cargo build --workspace
   ```
   Điều này sẽ tạo ra:
   - `yueex_engine.dll` (Engine ABI Bridge)
   - `yueex_dll.dll` (DLL Runtime để inject)

2. **Biên dịch Giao diện C# WPF**:
   ```powershell
   dotnet build YueEx.sln
   ```

3. **Chạy thử nghiệm**:
   - Khởi chạy tiến trình giả lập: `target\debug\mock_host.exe`
   - Khởi chạy UI: Chạy tệp `YueExUI.exe` được sinh ra sau khi build solution.
   - Click **Inject** trên UI để tiến hành manual map và tương tác viết script Luau.

---

## ⚠️ Khước Từ Trách Nhiệm (Disclaimer)
Dự án này được phát triển phục vụ cho mục đích nghiên cứu học thuật, kiểm thử bảo mật nội bộ và phát triển công cụ hỗ trợ phát triển sản phẩm chính thức. Việc sử dụng công cụ này chống lại các ứng dụng thương mại mà không có sự cho phép của chủ sở hữu là hoàn toàn bị cấm. Tác giả không chịu trách nhiệm cho bất kỳ thiệt hại hoặc vi phạm nào gây ra bởi dự án này."# YueExecutor" 
